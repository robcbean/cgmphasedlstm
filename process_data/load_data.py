...
class Scaler:
    ...
    def inverse_transform_value(self,_value):
        ...
        if self.scaler == ScalerValues.MinMaxScaler:
            ret = ret * (self.scaler_function.data_max_ - self.scaler_function.data_min_) + self.scaler_function.data_min_
...
class LoadData:
    ...
    def __init_parameters__(self,_filename,_scaler,_past_values,_future_steps,_time_range_minutes):
    ...
    def __check__parameters__(self,_filename=None, _scaler=None, _past_values=None, _future_steps=None,_time_range_minutes=None):
    ...
    def get_series_data(self, _raw_data, _field="", _value=""):
        ...
        if self.scaler != ScalerValues.NoScaler:
            values_scaled = self.scaler.scaler_function.fit_transform(values.reshape(-1, 1)).reshape(-1)
        else:
    ...
    def get_series_data_df(self,_raw_data):
        series_data_periodic = self.get_series_data(_raw_data, ExportFileFields.Type.value, RecordTypeOutput.Periodic.value)
        series_data_puntal = self.get_series_data(_raw_data, ExportFileFields.Type.value, RecordTypeOutput.Puntual.value,)
    ...
    def split_data_in_steps(self,_series_data):
        ...
        consecutive_segments = [c for c in consecutive_segments if len(c) >= self.past_values + self.future_steps]

        consecutive_segments_time = np.split(nd_glucose_index, idx_breaks.flatten())
        consecutive_segments_time = [d for d in consecutive_segments_time if len(d) >= self.past_values + self.future_steps]

        sups = [sequence_to_supervised(c, self.past_values, self.future_steps) for c in consecutive_segments]
        xss = [sup[0] for sup in sups]
        yss = [sup[1] for sup in sups]
        xs = np.concatenate(xss)
        ys = np.concatenate(yss)

        sups_t = [sequence_to_supervised(d, self.past_values, self.future_steps) for d in consecutive_segments_time]
    ...
    def find_prev_value(self,_cur_time, _series_data_extra):
        ...
        values = _series_data_extra[(_series_data_extra.index > prev_time_min) & (_series_data_extra.index < _cur_time)]
    ...
    def load_gluose_data_from_file(self, _filename,_scaler=ScalerValues.NoScaler,_past_values=12,_future_steps=1,_time_range_minutes=15):
        self.__check__parameters__(_filename,_scaler,_past_values,_future_steps,_time_range_minutes)
        self.__init_parameters__(_filename,_scaler,_past_values,_future_steps,_time_range_minutes)
...
def split_all_data(xs, xt, xt_t, ys, train_fraction, valid_fraction):
    ...
    return xs_train, xt_train,xt_t_train, ys_train, xs_valid, xt_valid,xt_t_valid, ys_valid, xs_test, xt_test, xt_t_test, ys_test
